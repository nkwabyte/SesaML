...
def run_train(args, config: PipelineConfig) -> None:
    ...
    try:
        ...
        train_collate = AudioCollator(text_transform, train_transforms, stride=config.model.stride)
        ...
        if args.val_dataset or args.val_split or args.val_csv_path:
            ...
            valid_collate = AudioCollator(text_transform, valid_transforms, stride=config.model.stride)
...
def run_evaluate(args, config: PipelineConfig) -> None:
    with RunManager(kind="evaluate", config=config, run_id=args.run_id, params=vars(args)) as run:
        ...
        collate_fn = AudioCollator(text_transform, valid_transforms, stride=config.model.stride)
